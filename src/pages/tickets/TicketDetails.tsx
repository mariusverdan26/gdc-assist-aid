...
import { getFirestore, doc, onSnapshot, Timestamp, updateDoc, serverTimestamp } from 'firebase/firestore';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { StatusBadge } from '@/components/ui/status-badge';
import { Calendar, ChevronLeft, Clock, MapPin, User, Loader2 } from 'lucide-react';
...
export default function TicketDetails() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, (snap) => {
      ...
      const acknowledgedAt: Date | undefined = (data.acknowledgeTime && typeof data.acknowledgeTime.toDate === 'function')
        ...
        : undefined;
      const resolvedAt: Date | undefined = undefined;
      ...
      const mapped: Ticket = {
        ...
        assignedTo: data.staffAssignedName ? { uid: '', name: data.staffAssignedName } : null,
  ...
  const meta: Array<{ label: string; value?: string; type?: 'status' }> | null = useMemo(() => {
  ...
  const canAcknowledge = isAdmin && ticket && ticket.status === 'pending' && !acknowledged;
  const canResolve = isAdmin && ticket && acknowledged && ticket.status !== 'resolved';
  ...
  const handleAcknowledge = async () => {
    ...
    try {
      ...
      toast({ title: 'Ticket acknowledged', description: 'Status updated to Ongoing.' });
    } catch (err) {
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      await updateDoc(ref, {
        status: 'Resolved',
        resolved: true,
        timeResolved: new Date().toLocaleString(),
      });
      toast({ title: 'Ticket resolved', description: 'Status updated to Resolved.' });
    } catch (err) {
  ...
  const openReassign = async () => {
    ...
    try {
      ...
      const list = snap.docs.map(d => {
        ...
        return { id: d.id, name: data.name || (data.email?.split('@')[0] ?? 'Unknown'), email: data.email };
      });
      setAdmins(list);
    } catch (e) {
      toast({ title: 'Failed to load admins', description: 'Please try again.', variant: 'destructive' });
    } finally {
      setLoadingAdmins(false);
    }
  };

  const handleReassignTo = async (admin: { id: string; name: string; email?: string }) => {
    if (!id) return;
    try {
      setUpdating(true);
      const db = getFirestore(app);
      ...
      toast({ title: 'Ticket reassigned', description: `Assigned to ${admin.name}.` });
    ...
    return (
      <div className="max-w-3xl mx-auto">
        <Card className="gdc-card">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">Please wait while we fetch the ticket details.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Full information for {ticket.ticketNo}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {canResolve && (
            <>
              ...
              <Button variant="outline" onClick={openReassign} disabled={updating}>
                Reassign Ticket
              </Button>
            </>
          )}
            ...
            {meta?.map((m) => (
              <div key={m.label} className="text-sm">
                ...
                <div className="font-medium">
                  {m.type === 'status' ? <StatusBadge status={ticket.status} /> : m.value}
                </div>
              </div>
            ))}
          </div>
      ...
      <Dialog open={isReassignOpen} onOpenChange={setIsReassignOpen}>
        <DialogContent>
          ...
          <div className="space-y-2 max-h-64 overflow-auto">
            {loadingAdmins ? (
              <div className="text-sm text-muted-foreground flex items-center"><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Loading admins...</div>
            ) : (
              admins.map((a) => (
                <div key={a.id} className="flex items-center justify-between border rounded-md p-2">
                  ...
                  <Button size="sm" onClick={() => handleReassignTo(a)} disabled={updating}>Assign</Button>
                </div>
              ))
            )}
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsReassignOpen(false)}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
